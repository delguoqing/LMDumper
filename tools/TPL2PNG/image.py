...
class TPL:
	...
	def toImage(self, outdir):
		...
		for i in xrange(header.ntextures):
			...
			if(tex.format == 0): #I4, 4-bit
				...
				print "Tex %d Format %d Palette %d w %d h %d" % (i, tex.format, palhead.format, w, h)
				if(palhead.format == 0):
					palette_data = self.fixPaletteIA8(tpldata)
				elif(palhead.format == 1):
					palette_data = self.fixPaletteRGB565(tpldata)
				elif(palhead.format == 2):
					palette_data = self.fixPaletteRGB5A3(tpldata)
				
				paldata = palette_data
				
				if(tex.format == 8):
					_w = (w + 7) / 8 * 8
					_h = (h + 7) / 8 * 8
					tpldata = struct.unpack(">" + str((_w * _h) / 2) + "B", data[tex.data_off:tex.data_off + ((_w * _h) / 2)])
					rgbdata = self.CI4((w, h), tpldata, paldata)
				if(tex.format == 9):
					_w = (w + 7) / 8 * 8
					_h = (h + 3) / 4 * 4
					tpldata = struct.unpack(">" + str(_w * _h) + "B", data[tex.data_off:tex.data_off + (_w * _h * 1)])
					rgbdata = self.CI8((w, h), tpldata, paldata)
				if(tex.format == 10):
					_w = (w + 3) / 4 * 4
					_h = (h + 3) / 4 * 4				
					tpldata = struct.unpack(">" + str(_w * _h) + "H", data[tex.data_off:tex.data_off + (_w * _h * 2)])
			...
			ext = "png"
			output.save(os.path.join(outdir, "noname_%d.png" % i), ext)
	...
	def CI4(self, (w, h), jar, pal):
		...
		bak_jar = jar
		jar = []
		for pixel in bak_jar:
			jar.append(pixel >> 4)
			jar.append(pixel & 0xF)
			...
			for x in xrange(0, w, 8):
				for y1 in xrange(y, y + 4):
					for x1 in xrange(x, x + 8):
						...
						out[y1 * w + x1] = rgba
		return ''.join(Struct.uint32(p) for p in out)
	def fixPaletteIA8(self, paldata):
		out = []
		for pixel in paldata:
			r = g = b = (pixel & 0xFF)
			a = ((pixel >> 8) & 0xFF)
			
			new_pixel = (r << 24) + (g << 16) + (b << 8) + (a << 0)
			out.append(new_pixel)
		return out
	def fixPaletteRGB565(self, paldata):
		out = []
		for pixel in paldata:
			r = ((pixel >> 11) & 0x1F) * 0x8
			g = ((pixel >> 5) & 0x3F) * 0x4
			...
			out.append(new_pixel)
		return out
	def fixPaletteRGB5A3(self, paldata):
		out = []
		for pixel in paldata:
			if(pixel & (1 << 15)): #RGB555
...
if __name__=='__main__':
	TPL(sys.argv[1]).toImage(sys.argv[2])